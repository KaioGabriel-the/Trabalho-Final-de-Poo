...
class App{
    ...
    private exibirFeed(): void {
        let publicacoesAux = this._redeSocial.publicacoes.sort((a, b) => b.data.getTime() - a.data.getTime());
    ...
    private vizualizarPerfilAvancado(){
        ...
        if(!(usuario instanceof PerfilAvancado)){
            console.log("O perfil não é avançado...");
            opcao = 0;
        }
        const menu = `--> ${usuario.apelido} <---\n\n` + 
        `--> Opções: \n` +
    ...
    private statusPerfilAvancado(usuario: Perfil){
        ...
        if(opcao === 1){
            ...
            let nomeUsuario = String(this._input("--> Digite o NOME do perfil que deseja alterar o status: "));
            enter(); cls();
            let usuarioProcurado = this._redeSocial.buscarPerfilPorApelido(nomeUsuario);
